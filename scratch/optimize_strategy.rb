...
module TradingBot
  class Simulator
    # Runs the backtest simulation with custom parameters
    def self.run_simulation(candles_hash, trial_params, config)
      all_trades = []
      ...
      candles_hash.each do |symbol, candles_1h|
        position = nil
        lookback = 100
        ...
        (lookback...candles_1h.size).each do |i|
          ...
          if position.nil?
            setup = detect_setup(slice_1h, ms, displacements, obs, sweeps, pd, current_price, atr_val, sl_mult, tp_mult, config)
            next unless setup
    ...
    def self.detect_setup(candles, ms, displacements, obs, sweeps, pd, price, atr, sl_mult, tp_mult, config)
...
sl_mult_grid.each do |sl|
  tp_mult_grid.each do |tp|
    max_hold_grid.each do |hold|
      ...
      metrics = TradingBot::Simulator.run_simulation(candles_hash, params, config)
