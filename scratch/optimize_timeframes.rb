...
module TradingBot
  class SimulatorTimeframe
    # Runs the backtest simulation using pre-computed indicators
    ...
    def self.detect_setup(step, sl_mult, tp_mult, config)
      ...
      recent_sweeps.each do |sweep|
        ...
        if is_ssl && (ms[:trend] != :bearish || in_discount)
          ...
          direction = "SHORT"
          sl = [price + atr * (sl_m = (sl_mult * 0.75)), ms[:protected_high] || price + atr * sl_mult].max
          tp = price - (sl - price) * tp_mult
        else
          next
        end
...
timeframes.each do |tf|
  ...
  symbols.each do |symbol|
    candles_hash[symbol] = TradingBot.fetch_historical(symbol, tf, start_date, end_date)
    puts "  Cached #{candles_hash[symbol].size} candles for #{symbol}"
  end

  puts "Pre-calculating technical indicators (SMC structures)..."
  cached_indicators = {}
  symbols.each do |symbol|
    ...
    (lookback...candles.size).each do |i|
      slice = candles[(i - lookback + 1)..i]
      pivots = PivotDetector.detect(slice, left_bars: 4, right_bars: 4)
      ms = MarketStructure.analyze(slice, pivots[:highs], pivots[:lows])
      atr_val = ATR.compute(slice)
      displacements = Displacement.detect(slice, atr_val)
      obs = OrderBlock.detect(slice, ms[:bos_events], displacements)

      eq = LiquiditySweep.find_equal_highs_lows(pivots[:highs], pivots[:lows])
      sweeps = LiquiditySweep.detect_sweeps(slice, pivots[:highs], pivots[:lows],
                                             equal_highs: eq[:equal_highs],
                                             equal_lows: eq[:equal_lows])

      recent_50 = slice.last(50)
      range_high = recent_50.map { |c| c[:high] }.max
      range_low  = recent_50.map { |c| c[:low] }.min
      pd = PDArray.compute(range_high, range_low)
      current_price = slice.last[:close]
      in_discount = PDArray.discount?(current_price, pd)

      cached_indicators[symbol] << {
        close: current_price,
        candle_count: slice.size,
        ms: ms,
        atr: atr_val,
        obs: obs,
        sweeps: sweeps,
  ...
  # Grid search params
  sl_mult_grid = [1.5, 2.5]
  tp_mult_grid = [2.5, 3.5]
  max_hold_grid = [24, 48]
  ...
  sl_mult_grid.each do |sl|
    tp_mult_grid.each do |tp|
      max_hold_grid.each do |hold|
        ...
        metrics = TradingBot::SimulatorTimeframe.run_simulation(cached_indicators, params, config)
  ...
  best_win = results.select { |r| r[:metrics][:total_trades] >= 30 }.max_by { |r| r[:metrics][:win_rate] }
...
puts "\n" + "=" * 60
puts "TIMEFRAME COMPARISON SUMMARY"
puts "=" * 60
timeframes.each do |tf|
  res = timeframe_results[tf]
  puts "⏰ TIMEFRAME: #{tf}"
  puts "  🏆 Best ROI:   #{res[:best_roi][:metrics]} (Params: #{res[:best_roi][:params]})"
  if res[:best_win]
    puts "  🎯 Best Win %: #{res[:best_win][:metrics]} (Params: #{res[:best_win][:params]})"
  else
    puts "  🎯 Best Win %: N/A (too few trades)"
  end
  puts "-" * 40
end
puts "=" * 60
